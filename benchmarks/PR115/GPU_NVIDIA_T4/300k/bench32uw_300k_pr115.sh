...
echo "=== Starting $i/30 `date` ($t1)"
taskset -c 0-7 time /root/unifrac/sucpp/ssu --mode partial --start $s1 --stop $s2  -i $BIOM -t $TREE -m unweighted_fp32 -n 1 -f -o /root/emp3_8f.out.1

t2=`date +%s`
echo "=== End $i/30 `date` ($t2)"
let dt=$t2-$t1
echo "Waltime $i: $dt"
...
rm -f /root/emp3_8f.out.1
echo "=== Starting $i/30 `date` ($t1)"
taskset -c 0-7 time /root/unifrac/sucpp/ssu --mode partial --start $s2 --stop 153619  -i $BIOM -t $TREE -m unweighted_fp32 -n 1 -f -o /root/emp3_8f.out.1

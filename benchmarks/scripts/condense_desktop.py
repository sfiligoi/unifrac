...
def parse_dir(dname):
  ...
  for fname in files:
    ...
    if h in data.keys():
      for k in f.keys():
        if k in data[h][1]:
          print("WARNING: [%s/%s] Duplicate %s %s"%(dname,fname,h,k))
        data[h][1][k] = f[k]
    else:
      data[h] = [n,f]
...
resnames=('org_cpu_i7','cpu_i7','gpu_1070','gpu_3090')
dnames={'org_cpu_i7':"org/CPU_Intel_Core_i7-10700_WSL",
        'cpu_i7':"v0.20.2/CPU_Intel_Core_i7-10700_WSL",
        'gpu_1070':"v0.20.2/GPU_NVIDIA_GTX1070",
        'gpu_3090':"v0.20.2/GPU_NVIDIA_RTX3090"}

rtypes={'unweighted': 'desktop_summary_uw.csv',
        'unweighted -f': 'desktop_summary_uw_f.csv',
        'weighted_normalized': 'desktop_summary_wn.csv',
        'weighted_normalized -f': 'desktop_summary_wn_f.csv'} 

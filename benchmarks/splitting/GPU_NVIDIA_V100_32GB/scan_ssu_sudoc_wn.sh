...
for ((ds=2032; $ds<25000; ds+=2032)); do
  ...
  taskset -c 0-7 time ssu -m weighted_normalized_fp32 -i sudoc_113k.biom -t sudoc_113k.tre -o /dev/shm/unifrac.tmp --mode partial --start $s1 --stop $s2
  rm -fr /dev/shm/unifrac.tmp
  s1=20320
  let s2=s1+ds
  echo "`date` ssu --start $s1 --stop $s2"
  ...
  t2=`date +%s`
  let dt=t2-t1
  echo "=== weighted_normalized fp32 ds: $ds time: $dt ==="
done

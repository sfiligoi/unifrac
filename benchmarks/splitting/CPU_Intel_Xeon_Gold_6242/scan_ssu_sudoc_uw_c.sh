...
t1=`date +%s`
for ((ds=6400; $ds<13000; ds+=1280)); do
  t1=`date +%s`
  s1=0
  let s2=s1+ds
  echo "`date` ssu --start $s1 --stop $s2"
  taskset -c 16-31 time ssu -m unweighted -i sudoc_113k.biom -t sudoc_113k.tre -o /dev/shm/unifrac.tmp --mode partial --start $s1 --stop $s2
  rm -fr /dev/shm/unifrac.tmp
  s1=20320
  let s2=s1+ds
  echo "`date` ssu --start $s1 --stop $s2"

...
for ((s1=0; $s1<153619; s1+=8000)); do
  ...
  taskset -c 0-7 time ssu -m unweighted_fp32 -i full_307k.biom -t full_307k.tre -o unifrac.tmp --mode partial --start $s1 --stop $s2
  ls -l unifrac.tmp
  rm -fr unifrac.tmp
done
t2=`date +%s`
...
for ((s1=0; $s1<153619; s1+=8000)); do
  ...
  echo "`date` ssu --start $s1 --stop $s2"
  taskset -c 0-7 time ssu -m weighted_normalized -i full_307k.biom -t full_307k.tre -o unifrac.tmp --mode partial --start $s1 --stop $s2
  ls -l unifrac.tmp
  rm -fr unifrac.tmp
done
t2=`date +%s`
...
for ((s1=0; $s1<153619; s1+=8000)); do
  ...
  echo "`date` ssu --start $s1 --stop $s2"
  taskset -c 0-7 time ssu -m weighted_normalized -f -i full_307k.biom -t full_307k.tre -o unifrac.tmp --mode partial --start $s1 --stop $s2
  ls -l unifrac.tmp
  rm -fr unifrac.tmp
done
t2=`date +%s`

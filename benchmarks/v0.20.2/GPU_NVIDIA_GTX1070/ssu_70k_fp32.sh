...
for ((i=0; $i<7; i+=1)); do
  ...
  taskset -c 0-7 time ssu -m unweighted_fp32 -i ../unifrac_70000.biom -t ../unifrac_70000.tre -o unifrac.tmp --mode partial --start $s1 --stop $s2
  rm -fr unifrac.tmp
  s1=$s2
done
t2=`date +%s`
...
for ((i=0; $i<7; i+=1)); do
  ...
  taskset -c 0-7 time ssu -m unweighted_fp32 -f -i ../unifrac_70000.biom -t ../unifrac_70000.tre -o unifrac.tmp --mode partial --start $s1 --stop $s2
  rm -fr unifrac.tmp
  s1=$s2
done
t2=`date +%s`
...
for ((i=0; $i<7; i+=1)); do
  ...
  taskset -c 0-7 time ssu -m weighted_normalized_fp32 -i ../unifrac_70000.biom -t ../unifrac_70000.tre -o unifrac.tmp --mode partial --start $s1 --stop $s2
  rm -fr unifrac.tmp
  s1=$s2
done
t2=`date +%s`

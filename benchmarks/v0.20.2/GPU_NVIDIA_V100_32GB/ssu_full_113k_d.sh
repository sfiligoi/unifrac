...
for ((s1=0; $s1<153619; s1+=8000)); do
  ...
  taskset -c 0-7 time ssu -m unweighted_fp32 -i full_307k.biom -t full_307k.tre -o /dev/shm/unifrac.tmp --mode partial --start $s1 --stop $s2
  rm -fr /dev/shm/unifrac.tmp
done
t2=`date +%s`
let dt=t2-t1
...
for ((s1=0; $s1<153619; s1+=8000)); do
  ...
  echo "`date` ssu --start $s1 --stop $s2"
  taskset -c 0-7 time ssu -m unweighted_fp32 -f -i full_307k.biom -t full_307k.tre -o /dev/shm/unifrac.tmp --mode partial --start $s1 --stop $s2
  rm -fr /dev/shm/unifrac.tmp
done
t2=`date +%s`
let dt=t2-t1
